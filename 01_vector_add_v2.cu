...
#define VECTOR_SIZE 10000000  // Vector size = 10 million
...
int main() {
    ...
    // Initialize vectors
    srand(time(NULL));
    init_vector(h_a, VECTOR_SIZE);
    init_vector(h_b, VECTOR_SIZE);
    ...
    int num_blocks_1d = (VECTOR_SIZE + BLOCK_SIZE_1D - 1) / BLOCK_SIZE_1D;
    ...
    for (int i = 0; i < 3; i++) {
        vector_add_cpu(h_a, h_b, h_c_cpu, VECTOR_SIZE);
        vector_add_gpu_1d << <num_blocks_1d, BLOCK_SIZE_1D >> > (d_a, d_b, d_c_1d, VECTOR_SIZE);
